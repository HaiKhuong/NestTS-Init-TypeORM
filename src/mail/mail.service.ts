  ...
  async userSignUp(mailData: MailData<{ hash: string }>) {
    try {
      ...
      await this.mailerService.sendMail({
        ...
        text: `${this.configService.get('app.frontendDomain')}/confirm-email/${mailData.data.hash} ${await i18n.t(
          'common.confirmEmail',
        )}`,
        template: 'activation',
        context: {
  ...
  async forgotPassword(mailData: MailData<{ hash: string }>) {
    try {
      ...
      await this.mailerService.sendMail({
        ...
        text: `${this.configService.get('app.frontendDomain')}/password-change/${mailData.data.hash} ${await i18n.t(
